...
-- @crossref:used-in[NK3 schema migration: api/migrations/008_data_migration_from_tdental_v3]
-- @crossref:uses[product-map/domains/settings-system.yaml, docs/MIGRATIONS.md, docs/TEST-MATRIX.md, testbright.md]
-- ============================================================================
-- Migration v3: Full data migration from old tdental DB to fresh tdental_demo DB
-- ============================================================================

-- SAFETY GUARD: this file truncates and rebuilds core data tables.
...
-- ============================================================================
-- 1. Create missing new-system tables that were lost during DB reset
-- ============================================================================

CREATE TABLE IF NOT EXISTS dbo.monthlyplans (
  id uuid PRIMARY KEY,
  name text,
  customer_id uuid REFERENCES dbo.partners(id),
  company_id uuid REFERENCES dbo.companies(id),
...
CREATE TABLE IF NOT EXISTS dbo.monthlyplan_items (
  id uuid PRIMARY KEY,
  plan_id uuid REFERENCES dbo.monthlyplans(id),
  invoice_id uuid,
  amount numeric,
  due_date date,
  status text,
...
CREATE TABLE IF NOT EXISTS dbo.planinstallments (
  id uuid PRIMARY KEY,
  plan_id uuid REFERENCES dbo.monthlyplans(id),
  amount numeric,
  due_date date,
...
CREATE TABLE IF NOT EXISTS dbo.payment_allocations (
  id uuid PRIMARY KEY,
  payment_id uuid REFERENCES dbo.payments(id),
  invoice_id uuid,
  amount numeric,
  target_type text,
  target_id uuid,
...
CREATE TABLE IF NOT EXISTS dbo.websitepages (
  id uuid PRIMARY KEY,
  title text,
  slug text,
  content text,
...
CREATE TABLE IF NOT EXISTS dbo.company_bank_settings (
...
-- ============================================================================
-- 2. Drop scaffold views and create real tables matching old DB schemas
-- ============================================================================
...
-- ============================================================================
-- 3. Fix base table constraints for old data compatibility
-- ============================================================================

ALTER TABLE dbo.appointments ALTER COLUMN timeexpected DROP NOT NULL;
ALTER TABLE dbo.appointments ALTER COLUMN isnotreatment DROP NOT NULL;
...
TRUNCATE TABLE dbo.company_bank_settings RESTART IDENTITY CASCADE;
...
-- Products: services only (229 rows)
...
-- Insert placeholder partner records for the 35 employees that have no partner match by ref
-- This ensures every migrated employee has a corresponding partner record for auth
INSERT INTO dbo.partners (
  id, name, email, phone, employee, active, companyid, isdeleted,
  customer, supplier, isagent, isinsurance, displayname
)
SELECT e.id, e.name, e.email, e.phone, true, e.active, e.companyid, false,
  false, false, false, false, e.name
FROM dblink('host=172.17.0.1 port=54323 dbname=tdental user=tdental password=TDental2026Dok!',
  $$SELECT e.id, e.name, e.email, e.phone, e.active, e.companyid
   FROM dbo.employees e
   LEFT JOIN dbo.partners p ON p.ref = e.ref AND p.employee = true
   WHERE p.id IS NULL$$)
AS e(id uuid, name text, email text, phone text, active boolean, companyid uuid)
ON CONFLICT (id) DO NOTHING;

-- Employees (377 rows) - use partner ID when matched, otherwise keep original employee ID
...
SELECT * FROM dblink('host=172.17.0.1 port=54323 dbname=tdental user=tdental password=TDental2026Dok!',
  $$SELECT 
   COALESCE(p.id, e.id) AS id, e.name, e.active, e.ref, e.address, e.phone, e.identitycard, e.email, e.birthday,
...
-- Agents (93 rows -> deduplicated because old DB has duplicate IDs)
INSERT INTO dbo.agents
SELECT DISTINCT ON (id) * FROM dblink('host=172.17.0.1 port=54323 dbname=tdental user=tdental password=TDental2026Dok!',
...
-- Assign all employee partners to Admin group (not employees table IDs)
