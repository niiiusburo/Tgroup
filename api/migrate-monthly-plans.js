...
async function main() {
  ...
  // Ensure all customers exist in target DB
  console.log('Checking for missing customers in target DB...');
  const allPartnerIds = [...new Set(Object.keys(groups).map(k => k.split('|')[0]))];

  // Check existing customers in batches
  const existingPartners = new Set();
  for (let i = 0; i < allPartnerIds.length; i += 500) {
    const batch = allPartnerIds.slice(i, i + 500);
    const existingRes = await target.query(
      `SELECT id FROM partners WHERE id = ANY($1)`,
      [batch]
    );
    for (const row of existingRes.rows) existingPartners.add(row.id);
  }

  const missingPartnerIds = allPartnerIds.filter(id => !existingPartners.has(id));
  console.log(`Found ${missingPartnerIds.length} missing customers. Importing stubs...`);

  if (missingPartnerIds.length > 0) {
    // Fetch names and company IDs from source
    const partnerInfoRes = await source.query(
      `SELECT id, name, companyid FROM partners WHERE id = ANY($1)`,
      [missingPartnerIds]
    );
    for (const p of partnerInfoRes.rows) {
      await target.query(`
      ...
      `, [p.id, p.name || 'Khách hàng chưa đặt tên', p.companyid]);
    }
    console.log(`Imported ${partnerInfoRes.rows.length} customer stubs`);
  }
