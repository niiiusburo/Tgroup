...
async function resolveInvestorScope(employeeId) {
  ...
  const groupName = String(groupRows[0]?.group_name || '').trim().toLowerCase();
  const isInvestor = groupRows.length > 0 && groupName === 'investor';
  if (!isInvestor) {
    return { isInvestor: false, allowedCustomerIds: [] };
  }

  const rows = await query(
    `WITH investor_account_ids AS (
       SELECT id
     ...
     WHERE is_visible = true
       ...
       AND (
         investor_id = $1
         OR investor_id IN (SELECT id FROM investor_account_ids)
       )`,
    [employeeId]
  );
