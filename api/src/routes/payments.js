...
router.post("/", requirePermission('payment.add'), validate(PaymentCreateSchema), async (req, res) => {
  ...
  try {
    ...
    // `amount` is passed so the allocations are also checked against the payment
    // total, not just against each target's residual.
...
router.post("/refund", requirePermission('payment.refund'), async (req, res) => {
  try {
    ...
    // The receipt number is minted by incrementing receipt_sequences. Generating it
    // outside a transaction meant a failed INSERT left the sequence advanced with no
    // payment behind it (a permanent gap in the receipt book). Both steps now share
    // one transaction, so a failure rolls the number back.
    const result = await withTransaction(async (tx) => {
      const receipt_number = await generateReceiptNumber("TUKH", tx);
...
router.patch("/:id", requirePermission('payment.edit'), validate(PaymentUpdateSchema), async (req, res) => {
  try {
    ...
    // Lowering `amount` used to be possible with no regard for allocations already
    // recorded against this payment, which silently created over-allocated rows
    // (INV-003). Re-check inside a transaction with the payment row locked so the
    // allocation total cannot change between the check and the update.
