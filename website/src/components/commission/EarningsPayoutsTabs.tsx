        ...
        <div className="flex gap-3">
          <label className="text-sm text-gray-600">Status
            <select value={status} onChange={(e) => setStatus(e.target.value)} className="block mt-1 px-3 py-2 border rounded-lg">
              <option value="pending">Pending</option><option value="paid">Paid</option><option value="reversed">Reversed</option><option value="">All</option>
            </select>
          </label>
          <label className="text-sm text-gray-600">LOB
            <select value={lob} onChange={(e) => setLob(e.target.value as any)} className="block mt-1 px-3 py-2 border rounded-lg">
              <option value="all">All</option><option value="dental">Dental</option><option value="cosmetic">Cosmetic</option>
            </select>
          </label>
        </div>
        <button onClick={handleLoad} className="px-4 py-2 rounded-lg bg-primary text-white text-sm font-medium hover:bg-primary-dark">Refresh</button>
      </div>
      {error && <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-2 text-sm text-red-700">{error}</div>}
      {loading ? <div className="p-8 text-center text-gray-500">Loading earnings...</div> : (
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-y border-gray-200"><tr>
              <th className="text-left px-4 py-3 font-medium text-gray-600">CTV</th><th className="text-left px-4 py-3 font-medium text-gray-600">Client</th><th className="text-left px-4 py-3 font-medium text-gray-600">Service</th><th className="text-left px-4 py-3 font-medium text-gray-600">LOB</th><th className="text-left px-4 py-3 font-medium text-gray-600">Level</th><th className="text-right px-4 py-3 font-medium text-gray-600">Amount</th><th className="text-left px-4 py-3 font-medium text-gray-600">Status</th>
            </tr></thead>
            <tbody className="divide-y divide-gray-200">
              ...
              {rows.length === 0 && <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500">No earnings found.</td></tr>}
            </tbody>
          </table>
        </div>
      )}
...
function PayoutsTab() {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-xl shadow-card p-6 space-y-4">
        <div className="flex flex-wrap gap-3 items-end justify-between">
          <label className="text-sm text-gray-600">Payout LOB
            <select value={lob} onChange={(e) => setLob(e.target.value as any)} className="block mt-1 px-3 py-2 border rounded-lg">
              <option value="cosmetic">Cosmetic</option><option value="dental">Dental</option>
            </select>
          </label>
          <button onClick={handleLoad} className="px-4 py-2 rounded-lg bg-gray-100 text-gray-700 text-sm font-medium hover:bg-gray-200">Reload</button>
        </div>
        {error && <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-2 text-sm text-red-700">{error}</div>}
        <div className="grid md:grid-cols-[1fr_auto] gap-3 items-end">
          <label className="text-sm text-gray-600">Cycle label
            <input value={cycleLabel} onChange={(e) => setCycleLabel(e.target.value)} placeholder="May 2026" className="block mt-1 w-full px-3 py-2 border rounded-lg" />
          </label>
          <button onClick={runPayout} disabled={!cycleLabel || selected.length === 0 || submitting} className="px-4 py-2 rounded-lg bg-primary text-white text-sm font-medium disabled:bg-gray-300">
            {submitting ? 'Processing…' : `Pay ${selected.length} (${formatVnd(totalSelected)})`}
          </button>
        </div>
        <div className="grid md:grid-cols-2 gap-3">
          <label className="text-sm text-gray-600">Notes
            <input value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Optional notes" className="block mt-1 w-full px-3 py-2 border rounded-lg" />
          </label>
          <label className="text-sm text-gray-600">Receipt photo
        ...
        {loading ? <div className="p-8 text-center text-gray-500">Loading payouts...</div> : (
          <div className="overflow-x-auto max-h-80">
            <table className="w-full text-sm"><thead className="bg-gray-50 border-y border-gray-200"><tr><th className="px-4 py-3" /><th className="text-left px-4 py-3 font-medium text-gray-600">CTV</th><th className="text-left px-4 py-3 font-medium text-gray-600">Client</th><th className="text-right px-4 py-3 font-medium text-gray-600">Amount</th></tr></thead><tbody className="divide-y divide-gray-200">
              ...
              {pending.length === 0 && <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-500">No pending earnings for this LOB.</td></tr>}
            </tbody></table>
          </div>
        )}
      </div>
      <div className="bg-white rounded-xl shadow-card overflow-hidden">
        <div className="px-6 py-4 font-semibold text-gray-900">Recent payout cycles</div>
        <table className="w-full text-sm">
          <tbody className="divide-y divide-gray-200">
            {payouts.map((p) => (
              <tr key={`${p.lob}-${p.id}`}>
                <td className="px-6 py-3">
                  <div className="flex items-center gap-3">
                    <div>
                      ...
                      <div className="text-xs text-gray-500">{p.lob} · {p.earnings_count} earnings · {p.paid_at ? new Date(p.paid_at).toLocaleDateString('vi-VN') : '—'}</div>
                    </div>
                  </div>
                </td>
            ...
            {payouts.length === 0 && <tr><td className="px-6 py-8 text-center text-gray-500">No payout cycles yet.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
