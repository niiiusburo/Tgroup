...
async def run_test():
    ...
    try:
        ...
        # -> Open the user menu (click avatar) so the logout option is visible, then click 'Đăng xuất' to log out and verify the app returns to /login.
        frame = context.pages[-1]
        # Click element
        ...
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/aside/div[2]/button[2]').nth(0)
        await asyncio.sleep(3); await elem.click()

        # -> Click the 'Đăng xuất' (logout) button to sign out, then wait for the app to redirect to /login and confirm the protected routes redirect.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/aside/div[2]/div/button[2]').nth(0)
        await asyncio.sleep(3); await elem.click()
