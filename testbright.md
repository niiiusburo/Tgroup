...
- Staff/admin and public `/checkin` users see only the light 3px preview blur instead of the previous 14px/video blur.
...
- [x] PASS: Live NK2 deployment proof shows `v0.32.51` on the staff header Face ID control and confirms NK production/NK3 are untouched - `https://nk2.2checkin.com/version.json` returned version `0.32.51`, gitCommit `ae5bda0`, branch `codex/nk2-investor-scope`; `/api/health` returned healthy with `checks.faceService: true` and `faceProvider: compreface`; live Playwright captured `output/playwright/nk2-face-live-20260629/live-customers-header-face-id-v03251.png`, `live-staff-face-modal-light-blur-v03251.png`, and `live-checkin-light-privacy-blur-v03251.png`; DOM proof showed staff/public overlays `bg-gray-950/10 backdrop-blur-[3px]` and `videoHasBlur: false`; `https://nk.2checkin.com/version.json` stayed `0.32.44` / `ff65634`; `tgroup-nk3-web`/`tgroup-nk3-api` were not restarted.
