...
# TestSprite Plan: CTV Referral Claim Service 2026-05-22

Feature/edit name: CTV Referral Claim Service — getReferralClaimStatus helper + computeClaim tests 2026-05-22

Changed URLs and API routes:
- No existing API routes changed.
- New backend service module created at `api/src/services/referralClaim.js` with exports: `computeClaim` (pure function), `getReferralClaimStatus` (async service), `addMonths` (helper).
- New Jest test suite created at `api/src/services/__tests__/referralClaim.test.js` with 14 comprehensive test cases.
- Future API endpoint expected: `POST /api/ReferralClaims` or similar to expose getReferralClaimStatus to frontend.

Affected data flows:
- Referral claim computation queries: `dbo.partners` (referred_by_ctv_id, name), `dbo.commission_settings` (referral_start_product_id), `dbo.saleorderlines` + `dbo.saleorders` (referral card date), `dbo.payments` (last paid service date).
- Claim active/inactive state determined by: anchorDate = max(referralCardDate, lastPaidServiceDate); expiresAt = anchorDate + 6 months; active = (expiresAt > asOf), inactive = (expiresAt <= asOf).
- Supports two database access patterns: direct getDb(lob) injection and transaction client (txClient) context.
- Null owner CTV ID returns inactive claim (null-safe computation).

Roles and scopes:
- Admin/staff viewing CTV commission and referral claim status.
- Partners with referred_by_ctv_id set (referral relationships) subject to claim expiration logic.
- Claim visibility scoped by location/branch (asOf defaults to current date in VN timezone).

Happy paths:
- Anchor date correctly selected as later of referral card date and last paid service date.
- Expiration calculated as exactly 6 months after anchor date.
- Claim active when asOf is before expiration date (strict > comparison).
- Claim inactive when asOf is at or after expiration date (boundary handled correctly).
- getReferralClaimStatus returns full claim object with ownerCtvId, ownerName, anchorDate, expiresAt, active fields.
- Database query patterns work with both getDb(lob) injection and txClient context.

Edge cases:
- Null owner CTV ID: getReferralClaimStatus returns inactive claim without querying further dates.
- Both referral card date and last paid service date null: anchor defaults to current date, claim expires 6 months from now.
- asOf parameter defaults to new Date() when not provided (handled via || new Date() in computeClaim).
- Boundary condition at exact expiration time: claim must be inactive (expiresAt.getTime() > asOf.getTime() with strict >).
- Single null date: anchorDate = Math.max() correctly uses the non-null date.
- Transaction client mocking: Jest mock chains .mockResolvedValueOnce() for sequential query results.

Regressions to prevent:
- Existing commission module tests continue passing.
- Date computation accuracy maintained (6-month window, boundary conditions).
- Database query patterns (both getDb and txClient) work without breaking existing code.
- Claim active/inactive state transitions consistent with business rules.

Setup data and login state:
- Test database: tdental_demo on port 5433 (Homebrew Postgres).
- Test clients: one with referral card date only, one with last paid service date only, one with both dates, one with null owner CTV ID.
- Test referral relationships: dbo.partners records with referred_by_ctv_id set to a valid CTV ID.
- Test commission settings: dbo.commission_settings with referral_start_product_id configured.

TestSprite execution items:
- [x] PASS: api/src/services/referralClaim.js created with computeClaim, getReferralClaimStatus, addMonths exports - verified by reading file.
- [x] PASS: api/src/services/__tests__/referralClaim.test.js created with 14 test cases covering all paths - verified by reading file.
- [x] PASS: All 14 referralClaim tests pass locally - `npm --prefix api test -- src/services/__tests__/referralClaim.test.js` returns 14 passing assertions.
- [x] PASS: docs/CHANGELOG.md updated with referral claim feature and test sections - verified by reading file.
- [x] PASS: product-map/test-matrix.md updated with referralClaim test entry - verified by reading file.
- [ ] PENDING: getReferralClaimStatus returns active claim when asOf is before expiration date (Jest test passes, need live integration verification).
- [ ] PENDING: getReferralClaimStatus returns inactive claim when asOf is at or after expiration date (Jest test passes, need live integration verification).
- [ ] PENDING: computeClaim correctly selects anchor as later of two dates (Jest test passes, need live integration verification).
- [ ] PENDING: Database query pattern works with both getDb injection and txClient context (Jest mock tests pass, need live database verification).
- [ ] PENDING: All 14 referralClaim tests pass in CI after merge.
- [ ] PENDING: Future API endpoint POST /api/ReferralClaims wires getReferralClaimStatus to frontend (future work).
- [ ] PENDING: NK live integration test: getReferralClaimStatus with real partner referral relationship returns correct active/inactive state.
