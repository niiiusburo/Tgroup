...
| Receptionist/Admin/Super Admin/Dentist/Assistant role access matrix. | Partially completed at the backend permission boundary. Payment create/refund/void/delete and Hồ sơ online view/create/upload are now split into separate permission strings; the migration keeps assistant-facing upload access while blocking patient creation for normal clinic roles. Full role fixture seeding/browser matrix still needs production-role verification. | Covered by `readRoutePermissions.test.js`; role-by-role browser testing remains in `testbright.md`. |
...
| Hồ sơ online can view but cannot upload. | Completed at the permission boundary. Backend upload now requires `external_checkups.upload`, patient creation remains `external_checkups.create`, and the customer profile hides create/upload controls unless the matching permission is present; assistant-facing roles are granted upload by migration for the sheet requirement. | Covered by `readRoutePermissions.test.js` and `HealthCheckupGallery.permissions.test.tsx`; Dental Assistant browser verification remains in `testbright.md`. |
