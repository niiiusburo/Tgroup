...
| `dbo.Employees` / staff partners | `dbo.partners` employee | existing local employee by source `Id`/`PartnerId` | normalized name, then alias-normalized name | Reuse existing staff first. If the same staff exists at another branch, add the missing branch to `employee_location_scope` instead of creating a duplicate employee. Alias normalization collapses spacing and punctuation, so values such as `AnhVL saleonline` / `AnhVL Sale online` and `Bác sĩ 17` / `Bác sĩ (17)` map to one staff identity. Create staff only when no confident match exists. |
