...
- `/api/Exports` report and operational workbook preview/download builders

Contract invariant: non-investor employees keep existing unscoped behavior except where cross-location report/export endpoints explicitly enforce employee location scope. Investor employees with an empty allowlist receive empty/404 scoped results. The investor group resolves to explicit staff-shell permissions without wildcard `*`; payment legacy fallback reads may run only for checked customers, report branch/date filters compose with the checked-customer allowlist, customer-linked Excel exports apply the same allowlist before rows are built, and permission/employee self-escalation plus investor allowlist curation remain server-blocked.

Reports/export location invariant: `/api/Reports/*` and operational `/api/Exports/*` builders must reject an explicit out-of-scope `companyId`, narrow omitted all-location requests to the employee's allowed locations, and fail closed when a non-admin/non-investor report cannot be scoped to any location.
...
| Method | Path | Auth | Body / Query | Response |
|--------|------|------|--------------|----------|
| POST | `/recognize` | Perm:`customers.view` | FormData (`image`) | `{ status, match: FaceCandidate \| null, candidates: FaceCandidate[], ambiguity, recognitionVersion }`; provider selected by `FACE_RECOGNITION_PROVIDER=local|compreface`; `status:"ambiguous"` (best two plausible identities closer than `FACE_AMBIGUOUS_MATCH_MARGIN`, default `0.06`) is rescan-only with `match: null` and empty `candidates`; staff responses expose NK2 Face ID version metadata such as `face-recognition-0.32.59`; strict defaults are auto-match `0.92`, candidate `0.84`, margin `0.05`; writes hidden server-side Face ID diagnostics when enabled and never returns `privateDiagnostics` |
...
| GET | `/status/:partnerId` | Perm:`customers.view` | — | `{ partnerId, registered, sampleCount, lastRegisteredAt, provider?, readiness }`; CompreFace mode verifies actual `/faces?subject=` example count and readiness uses sample coverage; local/SFace can blend active embedding `detection_score` |
