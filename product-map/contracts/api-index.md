...
|--------|------|------|------|----------|
| POST | `/login` | Public | `{ email, password }`; investor emails may resolve through active `dbo.investor_accounts` and a case-insensitive `investor` permission group match | `{ token, user (with is_ctv?, lob_scope?), permissions }` |
...
| POST | `/:id/source-correction` | Perm:`customers.source_correct` | `{ new_sourceid, expected_old_sourceid, reason, correction_manifest_ref }` | `{ partner, audit }` authorized partner source correction (only approved partner source write path); investor accounts refused `404` by the mounted `requireNonInvestorPermission` guard, before the permission comparison and before the handler acts on the body, so the response is identical with or without `customers.source_correct` (D21/INV-021) |
...
| PATCH | `/:id` | Perm:`customers.edit` | Order fields; quantity, service, tooth, and price fields also sync to the primary rendered sale-order line. Clients omit `sourceid` on unrelated edits; an explicitly submitted inactive source is allowed only when directly assigned to this same order. Paid/closed-period source changes are rejected with `409 SOURCE_IMMUTABLE` (repeat current value is a no-op). | Updated sale order; `400 CUSTOMER_SOURCE_NOT_SELECTABLE` for a new inactive/missing source; `409 SOURCE_IMMUTABLE` when locked; a PATCH whose only field is the current `sourceid` returns `200` with the unchanged order and zero UPDATE/audit (locked or not), while a PATCH with no fields at all still returns `400 No fields to update`; successful open-order source changes write one `source_change_audit` row (`api_patch`) |
| POST | `/:id/source-correction` | Perm:`services.source_correct` | `{ new_sourceid, expected_old_sourceid, reason, evidence, rollback_reference, correction_manifest_ref? }` | `{ order, correction, audit }` authorized correction in both correction and append-only ledgers (`change_channel=source_correction`); investor accounts refused `404` by the mounted `requireNonInvestorPermission` guard, before the permission comparison and before any lock or write, so the response is identical with or without `services.source_correct` (D21/INV-021) |
