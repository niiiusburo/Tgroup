...
|-----------|-------|
| **Primary Key** | `id` (uuid) |
| **Key Relationships** | `partner_id` → partners(id), where the partner row is the canonical investor identity. Migration 049 normalizes legacy `is_active`/`created_at`/`updated_at` into `active`/`datecreated`/`lastupdated`. |
