...
| `components/ProtectedRoute/__tests__/protectedRoute.ctvRedirect.test.tsx` | Auth | AuthUser type retains optional `is_ctv` and `lob_scope` fields; ProtectedRoute contains no `is_ctv` check or `/ctv` navigation until the LOB route ships; backward compatibility with undefined fields verified |
