...
| `saleOrders.test.js` | Services/Payments | Sale order edits recalculate residual display from `payment_allocations`; source semantics (direct order vs customer, create snapshot, no inherited rewrite) |
| `orderSourceSemantics.test.js` | Services/Reports/Exports | INV-023 helper: no COALESCE into sourceid; legacy_coalesce_v1 versioned only |
| `mapSaleOrderToServiceRecord.test.ts` | Services | Frontend never maps customersourceid into sourceId |
...
| `src/routes/reports/__tests__/revenueRecognition.test.js` | Reports/Payments | UC-013/WF-013/INV-023: `CON-Reports-RevenueSummary` plus trend/doctor/category/by-source revenue; posted service payment allocations, deposit exclusion, allocation proration, payment-date recognition; by-source uses immutable order source only |
...
| `src/services/exports/__tests__/legacyFlatReportsExport.test.js` | Reports/Exports | UC-013/WF-005/INV-023: `revenue-flat` and `deposit-flat` registry, workbook templates, SO-code column mapping, separate Nguồn đơn / Nguồn KH (no COALESCE), posted service-payment filters, allocation proration SQL, deposit top-up filtering, and row-limit error |
