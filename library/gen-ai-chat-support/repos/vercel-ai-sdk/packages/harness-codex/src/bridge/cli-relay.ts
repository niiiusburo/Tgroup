/*
 * Compatibility shim for upstream codex CLI issue
...
export function isToolRelayCommand({
  ...
  const directPrefixes = [
    `node ${cliShimPath}`,
    `node ${shellQuote(cliShimPath)}`,
  ];
  return directPrefixes.some(
    prefix =>
