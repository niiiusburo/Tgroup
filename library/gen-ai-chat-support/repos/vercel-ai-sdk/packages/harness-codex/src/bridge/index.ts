...
// Codex exec-mode compatibility relay (#19425). MCP remains configured below;
// these helpers are only activated for host tools and preserve relay evidence.
import {
  CLI_SHIM_FILENAME,
  buildCliShimScript,
...
async function runTurn(start: StartMessage, turn: BridgeTurn): Promise<void> {
   ...
   *
   * Until that's fixed, host tools are made available to the model via the
   * CLI relay documented in `./cli-relay.ts`. The MCP server config below is
   * kept as the primary integration point so that the day codex starts
   * exposing MCP tools properly, host tools work both ways. Three hookpoints
   * in this file (writeFile for the shim, composeUserMessage's
   * toolUsageBlock, and the isToolRelayCommand filter in the event loop)
   * implement the relay and can be removed once the upstream bug is fixed.
   */
  ...
  if (start.tools && start.tools.length > 0) {
    ...
    // Compatibility/fail-safe: the per-turn CLI shim uses the same loopback
    // bearer-token relay as MCP, so host tool calls still emit tool-call and
    // tool-result events while codex exec mode lacks model-callable MCP tools.
  ...
  const userMessage = composeUserMessage({
    ...
    // Compatibility/fail-safe: instructions are injected only when the shim was
    // written for this turn; otherwise the prompt contains no alternate tool
    // path and the bridge relies solely on native/MCP behavior.
    toolUsageBlock:
  ...
  try {
    ...
    for await (const event of events as AsyncIterable<CodexEvent>) {
      ...
      // Compatibility/fail-safe: hide only direct shim invocations. Unexpected
      // shell commands that merely mention the shim path remain visible in the
      // event stream for debugging instead of being broadly swallowed.
      if (
        cliShimPath &&
