...
### Security
- ExternalCheckups customer-code reads/mutations and image-by-name proxy now call `resolveInvestorScope` and fail closed (404, no PII) outside `dbo.investor_clients`; list image URLs carry `?customerCode=` and filename T-code extraction is the fallback — @agent — AUD-005 / INV-021.
...
### Testing
- Added `api/tests/externalCheckupsInvestorScope.test.js` and `api/tests/stockPickingsInvestorScope.test.js` IDOR coverage; extended `investorScopeRoutePermissions.test.js` for StockPickings GET gates — @agent — AUD-005.
