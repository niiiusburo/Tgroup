...
### Tests
- `api/src/services/__tests__/referralClaim.test.js` — 14-assertion Jest test suite for the new referral claim service. Tests `computeClaim` pure function (null owner handling, anchor date selection as the later of referral card date and last paid service date, 6-month expiration window calculation, active/inactive state transitions based on asOf parameter, and default asOf to current date). Tests `getReferralClaimStatus` async database service (database query mocking with jest.fn(), support for both direct getDb injection and transaction client patterns, null/missing value handling). Verifies that claims become inactive exactly at the expiration date boundary (strict greater-than comparison). — @agent — 2026-05-22 establishes test coverage for CTV referral claim computation and database integration.

### Added
- `api/src/services/referralClaim.js` — New service module for CTV referral claim tracking. Exports `computeClaim` pure function that determines claim status from owner CTV ID and date inputs (anchor date is the later of referral card date or last paid service date, expiration is 6 months after anchor, active state depends on asOf parameter). Exports `getReferralClaimStatus` async function that queries the database for owner info, referral card date, and last paid service date, then returns the computed claim. Supports both direct database access (via injected getDb function) and transaction client context (txClient pattern). Integrates with `dbo.partners` (referred_by_ctv_id, name), `dbo.commission_settings` (referral_start_product_id), `dbo.saleorderlines` + `dbo.saleorders` (referral card date), and `dbo.payments` (last paid service date). — @agent — 2026-05-22 enables commission module to track which CTVs have active referral claims.
