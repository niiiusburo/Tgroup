...
| v1.0.47 | 2026-07-03 | Employee frontend role-inference contract clarified: `GET /api/Employees` still returns raw migrated flags, and the frontend derives selector roles. Assistant rows whose normalized title is `Trợ lý bác sĩ` / `Tro ly bac si` classify as `doctor-assistant` before generic doctor, so appointment TLBS selectors show NK migrated staff that have both doctor and assistant flags. API payload shape unchanged. |
...
Employee selector role contract: `GET /api/Employees` returns migrated `dbo.partners` staff flags as-is. Frontend consumers derive exactly one selector role from those flags and title fields. If `isassistant=true` and the normalized `jobtitle`/`hrjobname` reads `Trợ lý bác sĩ` / `Tro ly bac si`, the derived role is `doctor-assistant` even when `isdoctor=true`; otherwise normal doctor, receptionist, assistant, and title-only classifications remain unchanged. This keeps appointment/service TLBS dropdowns aligned with NK migrated staff data without mutating the database.
