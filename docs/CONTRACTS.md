...
| v1.0.48 | 2026-07-03 | Reports location-scope contract hardened: all branch-sensitive `/api/Reports` endpoints resolve `employee_location_scope` before SQL. Plain `Admin` report reads are limited to assigned locations, wildcard / `Super Admin` / `System Administrator` keep all-location reads, and explicit out-of-scope `companyId` requests return 403 before report queries run. |
...
```
Date-scoped endpoints accept `{ dateFrom?: 'YYYY-MM-DD'; dateTo?: 'YYYY-MM-DD'; companyId?: string }` unless noted. Branch-sensitive report endpoints must apply backend location scope through the caller's effective `employee_location_scope`: plain `Admin` users only receive rows for assigned locations, wildcard / `Super Admin` / `System Administrator` users can receive all locations, and an explicit out-of-scope `companyId` returns `403 { success:false, error:'Location not allowed' }` before report SQL runs.
