...
| 048 | `048_investor_customer_scope.sql` | Creates investor customer allowlist and seeds read-only/list-only investor group | `CREATE TABLE IF NOT EXISTS dbo.investor_clients`; seed `investor` permission group with view permissions plus scoped-list `customers.view_all` | Remove investor assignments first, delete seeded group permissions/group if unused, then `DROP TABLE IF EXISTS dbo.investor_clients` after backing up allowlist rows | 2026-06 |
