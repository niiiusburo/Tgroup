...
| 048 | `048_investor_customer_scope.sql` | Creates investor customer allowlist and seeds explicit staff-shell investor group without wildcard access | `CREATE TABLE IF NOT EXISTS dbo.investor_clients`; seed `investor` permission group with staff-shell permission strings while `resolveInvestorScope()` enforces customer allowlist scope | Remove investor assignments first, delete seeded group permissions/group if unused, then `DROP TABLE IF EXISTS dbo.investor_clients` after backing up allowlist rows | 2026-06 |
| 049 | `049_investor_accounts_nk2_credentials.sql` | Creates NK2-only investor credential table for shared NK/NK2 databases | `CREATE TABLE IF NOT EXISTS dbo.investor_accounts`; unique lower(email) index; partner FK | Disable account rows first, back up credentials metadata, then `DROP TABLE IF EXISTS dbo.investor_accounts` only after NK2 no longer uses the credential fallback | 2026-06 |
