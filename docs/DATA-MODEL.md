...
Investor identities remain `dbo.partners` employee rows assigned to the `investor` permission group. Optional alternate investor login credentials live in `dbo.investor_accounts` but resolve back to the same `partners.id` and normal staff JWT/session.
