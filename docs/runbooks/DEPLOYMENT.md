...
5. If schema or permission data changed, apply and verify migrations locally first. **After deploy, also apply unapplied canonical migrations on the VPS DB** — migrations are not auto-run. Use the **non-recursive** top-level glob `api/migrations/*.sql` only to discover candidates; never bulk re-run it. Confirm each exact file is unapplied using `dbo.schema_migrations` and the current schema, review its rollback/idempotency notes in `docs/MIGRATIONS.md`, then invoke that file explicitly. Active migration history is not uniformly idempotent. Symptom of a missed migration: API returns 500 with `relation "dbo.<table>" does not exist`.
   - `api/migrations/` is the canonical deploy path. **Never** include `api/migrations/RETIRED-DESTRUCTIVE-DO-NOT-RUN/**` in a deployment or migration loop (`.sql.retired` forensic artifacts, including 008 TDental TRUNCATE bulk imports — AUD-001). Exceptional disposable-target execution is deployment-captain-approved only; follow that folder's README and `docs/MIGRATIONS.md`.
   - `api/src/db/migrations/` currently contains supplemental stragglers (`003_add_payment_category.sql`, `046_customer_face_embeddings.sql`); check `docs/MIGRATIONS.md` and run/consolidate those explicitly when a change depends on them.
