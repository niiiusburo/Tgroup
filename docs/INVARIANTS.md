...
**Enforced by:** `resolveInvestorScope()` in `api/src/services/permissionService.js`, route-level customer scope filters, `api/src/services/reports/canonicalRevenue.js` `buildWhere()` (empty `allowedCustomerIds` and bare `isInvestor` fail closed — never omit the partner filter), `api/src/services/__tests__/permissionService.test.js`, `api/src/services/reports/__tests__/canonicalRevenue.test.js`, `api/tests/authInvestorLogin.test.js`, `api/tests/investorIdorScoping.test.js`, `api/tests/investorScopeRoutePermissions.test.js`, and `api/tests/investorVisibilityCompatibility.test.js`.
