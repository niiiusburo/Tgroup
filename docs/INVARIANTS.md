...
**Enforced by:** `resolveInvestorScope()` and `INVESTOR_STAFF_PERMISSIONS` in `api/src/services/permissionService.js`, investor guards in customer/payment/appointment/face-recognition routes, `api/src/routes/auth.js` investor credential fallback, migrations `048_investor_customer_scope.sql` and `049_investor_accounts_nk2_credentials.sql`, focused investor auth, IDOR, route-permission, face-recognition, and admin-mutation tests.
