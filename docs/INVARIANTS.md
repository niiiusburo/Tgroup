...
**Enforced by:** `resolveInvestorScope()` in `api/src/services/permissionService.js`, route-level customer scope filters (including ExternalCheckups list/image/mutations and StockPickings GET partner scope), `api/src/services/__tests__/permissionService.test.js`, `api/tests/authInvestorLogin.test.js`, `api/tests/investorIdorScoping.test.js`, `api/tests/investorScopeRoutePermissions.test.js`, `api/tests/externalCheckupsInvestorScope.test.js`, `api/tests/stockPickingsInvestorScope.test.js`, and `api/tests/investorVisibilityCompatibility.test.js`.
**Cite when:** Editing investor auth, customer reads, external checkups, stock pickings, reports, exports, appointments, payments, service cards, or permission seeds.
