...
| If you change... | Run these tests... | Why |
|---|---|---|
| `api/src/routes/payments.js` | `api/tests/readRoutePermissions.test.js`, `api/tests/paymentsTransaction.test.js`, `api/tests/paymentPatchVoidBan.test.js`, `api/tests/paymentReversalGuards.test.js`, `api/tests/paymentAllocationConcurrency.integration.test.js`, `website/e2e/team-charlie-payments.spec.ts`, `website/src/lib/allocatePaymentSources.test.ts` | Money logic is high-risk. AUD-006 transaction/race matrix + AUD-009 create/PATCH void bans + reversal lock order. |
| `api/src/routes/payments/helpers.js` | `api/tests/paymentAllocationGuards.test.js`, `api/tests/paymentReversalGuards.test.js`, `api/tests/paymentAllocationConcurrency.integration.test.js`, `api/tests/paymentsTransaction.test.js`, `website/src/lib/allocatePaymentSources.test.ts` | Positive allocation validation, allocation math, PostgreSQL two-connection FOR UPDATE behavior, stable create/reverse lock order, multi-alloc sum, receipt generation. Set `TEST_DATABASE_URL` to run the database matrix. |
...
| `api/src/routes/saleOrders/updateSaleOrder.js` amount/residual handling | `api/tests/saleOrders.test.js`, `api/tests/paymentAllocationConcurrency.integration.test.js` | Sale-order amount edits must lock before allocation reads and share the payment writer's lock order. |
...
|---|---|---|
| **Payments (backend allocation/void/refund)** | Positive allocation amounts, residual FOR UPDATE, multi-alloc sum, stable reversal locks, and create/PATCH void bans are covered. The real two-connection invoice/dotkham matrix runs when `TEST_DATABASE_URL` is configured. Refund balance checks remain thinner. | **Medium** — core allocation/reversal guards are locked; refund math still worth expanding. |
