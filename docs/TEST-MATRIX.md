...
| `api/src/routes/faceRecognition.js` | `api/tests/faceRecognition.test.js`, `api/tests/faceRecognitionInvestorScope.test.js` | Face register/re-register/recognize API contract in local and CompreFace modes; investor fail-closed ranking and response allowlist on recognize/status plus role-level register/re-register denial. |
| `api/src/services/faceMatchEngine.js` / `api/src/services/comprefaceFaceProvider.js` | `api/src/services/__tests__/faceMatchEngine.test.js`, `api/src/services/__tests__/comprefaceFaceProvider.test.js` | Local SQL and CompreFace partner hydration constrain investor ranking to allowed customer ids; empty allowlists fail closed before customer hydration. |
