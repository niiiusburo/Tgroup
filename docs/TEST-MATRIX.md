...
| `api/src/routes/ctv.js` self-dashboard reads | `api/src/__tests__/ctvRouteGating.test.js`, `api/src/routes/__tests__/ctvReferrals.test.js`, live `GET /api/ctv/me` as admin must return 403 `S_CTV_ONLY` | CTV commission/referral/profile reads are cross-DB and must remain CTV-only even when admins have broad permissions; referral service rows must stay scoped to the current CTV. |
| `website/src/pages/CTV/*` and `website/src/components/ctv/*` | `website/src/pages/CTV/index.test.tsx`, `website/src/components/ctv/ReferralFlipCard.test.tsx`, `/ctv` mobile screenshot as a CTV user | CTV portal search/filter/flip-card behavior is the self-service CTV surface and must keep service lists readable on mobile. |
