...
| `api/src/routes/partners/resolveHandler.js` | `api/src/routes/partners/__tests__/resolveHandler.test.js`, `api/tests/investorIdorScoping.test.js` | AUD-003: resolve applies investor allowlist fail-closed, excludes soft-deleted partners, and principal/scope-scopes the cache so staff hits cannot leak name/phone to investors. |
