...
| `api/migrations/` 008 TDental TRUNCATE bulk imports or default migration glob safety | `npm --prefix api test -- --runTestsByPath tests/destructiveTruncateMigrationArchiveGuard.test.js --runInBand` (required by `.github/workflows/pr-checks.yml`) | One-shot TRUNCATE imports must stay quarantined as `.sql.retired`; active root `api/migrations/*.sql` files must contain no TRUNCATE statement. |
