...
| `api/src/routes/faceCheckin.js` / `website/src/pages/CheckIn/CheckIn.tsx` | `api/src/routes/__tests__/faceCheckin.test.js`, `website/src/pages/CheckIn/CheckIn.test.tsx` | Public no-auth kiosk route, front-camera startup, privacy-minimized match response, rate limiting, and public page rendering without AuthProvider. |
