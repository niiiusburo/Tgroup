...
| `api/src/server.js` `GET /api/health`, `api/src/services/faceRecognitionRuntime.js` | `api/tests/health.test.js`, `api/src/services/__tests__/faceRecognitionRuntime.test.js` | DB-required vs optional face degrade, including bounded never-settling local and CompreFace probes (INV-014 / AUD-038). |
