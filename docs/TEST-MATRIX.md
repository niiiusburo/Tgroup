...
| `api/src/routes/partners/mutationHandlers.js` | `api/src/routes/partners/__tests__/mutationHandlers.test.js`, `api/tests/partnerMutationsPasswordHash.test.js` | New dental customers must keep `T######`; new cosmetic customers created through `/api/cosmetic/Partners` must use collision-checked `TM######` refs in the cosmetic DB; partial updates preserve omitted writable UUIDs; normal Partner POST/PUT rejects source assignment/change/clear with `PARTNER_SOURCE_READ_ONLY`; route validation accepts an unchanged UUID regardless of letter case; normal PUT, compatibility no-op, and soft-delete receipts omit password fields. |
...
| If you change... | Run these tests... | Why |
|---|---|---|
| `api/src/routes/employees/mutations.js` | `api/tests/employeeMutationsPasswordHash.test.js` | POST/PUT must never return `password_hash`/`password` while still bcrypt-hashing password writes; missing PUT targets return 404 before commit or tier/location side effects (AUD-011). |
