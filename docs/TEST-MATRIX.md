...
| `api/src/routes/faceCheckin.js` / `website/src/pages/CheckIn/CheckIn.tsx` | `api/src/routes/__tests__/faceCheckin.test.js`, `website/src/pages/CheckIn/CheckIn.test.tsx`, `website/src/components/shared/faceCaptureEngine.test.ts` | Public no-auth kiosk route, front-camera startup, visible camera flip, privacy-softened preview, center-cropped provider capture, privacy-minimized match response, rate limiting, and public page rendering without AuthProvider. |
