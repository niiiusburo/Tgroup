...
| `website/src/pages/Commission.tsx`, `website/src/components/commission/*`, `api/src/services/newClientsQuery.js`, `api/src/services/exports/builders/newClientsExport.js`, `website/src/pages/Customers.tsx`, `website/src/components/customer/ServiceHistory*.tsx` | `npm --prefix website test -- --run CommissionNavigation NewClientsTab EarningsPayoutsTabs ServiceHistory`; `cd api && JWT_SECRET=test npx jest --runInBand --runTestsByPath src/services/__tests__/newClientsQuery.test.js`; website build/typecheck; live `/commission?tab=newClients&lob=cosmetic` and `/commission?tab=earnings&lob=cosmetic` screenshot smoke | Admin CTV commission navigation must preserve the active tab in the URL, link new-client names to profiles, show referral service revenue/paid/COM/missing-COM status, link earning/payout services to focused customer Records rows, and keep return breadcrumbs intact on mobile. |
