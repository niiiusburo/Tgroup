...
| `api/src/routes/ctv.js` or `api/src/routes/ctvPublic.js` referral booking/sign-up writes | `api/src/routes/__tests__/ctvBookings.test.js`, `api/src/routes/__tests__/ctvPublicJoin.test.js`, `api/src/services/__tests__/referralClaim.test.js`, `website/src/components/ctv/CtvReferModal.test.tsx`, `website/src/pages/CTV/JoinCtv.test.tsx`, `website/src/lib/api/__tests__/ctv.booking.test.ts`, admin `/customers?lob=cosmetic` search smoke for the accepted client after deploy | CTV booking can reclaim an existing partner row; accepted clients must be marked `customer=true` so admin customer search finds them without duplicate identities. Authenticated and public landing bookings must create an appointment only, never a saleorder/service card; omitted service metadata defaults to Referral Start when configured, phone lookup may prefill the name only for an available existing client, and public mode must live-verify a valid active CTV by phone before submit. Public CTV signup must live-verify the upline phone, create the new CTV under a valid referral code or active upline CTV phone, and reject missing/unknown uplines before writing a partner. |
