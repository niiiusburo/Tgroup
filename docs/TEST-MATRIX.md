...
| If you change... | Run these tests... | Why |
|---|---|---|
| `api/src/routes/payments.js` | `api/tests/readRoutePermissions.test.js`, `api/tests/paymentsTransaction.test.js`, `api/tests/paymentPatchVoidBan.test.js`, `api/tests/paymentReversalGuards.test.js`, `npm --prefix api run test:payment-concurrency`, `website/e2e/team-charlie-payments.spec.ts`, `website/src/lib/allocatePaymentSources.test.ts` | Money logic is high-risk. AUD-006 transaction/race matrix + AUD-009 create/PATCH void bans + reversal lock order. The PR Checks PostgreSQL job supplies `TEST_DATABASE_URL`. |
| `api/src/routes/payments/helpers.js` | `api/tests/paymentAllocationGuards.test.js`, `api/tests/paymentReversalGuards.test.js`, `npm --prefix api run test:payment-concurrency`, `api/tests/paymentsTransaction.test.js`, `website/src/lib/allocatePaymentSources.test.ts` | Positive allocation validation, allocation math, PostgreSQL two-connection FOR UPDATE behavior, stable create/reverse lock order, multi-alloc sum, receipt generation. The dedicated concurrency command fails when `TEST_DATABASE_URL` is missing. |
| `api/src/routes/payments/reversalHandlers.js` | `api/tests/paymentReversalGuards.test.js` | Delete and void must lock the payment and all allocation targets before any reversal write. |
| `api/src/routes/saleOrders/updateSaleOrder.js` amount/residual handling | `api/tests/saleOrders.test.js`, `npm --prefix api run test:payment-concurrency` | Sale-order amount edits must lock before allocation reads and share the payment writer's lock order; PR Checks enforces the database race matrix. |
...
| `api/src/db.js`, `api/src/routes/customerSources.js`, `api/src/routes/saleOrders/customerSourceSelection.js`, or sale-order create/update source handling | `npm --prefix api test -- --runTestsByPath tests/dbTransaction.test.js tests/customerSourceIntegrity.test.js tests/customerSourceReferenceMigration.test.js tests/q10CustomerSourceRepairGuard.test.js --runInBand` | New data must reject inactive/missing sources; source management and order writes must serialize on the lookup row; existing history may preserve only its exact source; referenced lookups cannot be deleted. |
...
| `website/src/hooks/useSettings.ts`, `website/src/hooks/useServices.ts`, or ServiceForm customer-source choices | `npm --prefix website test -- src/hooks/useSettings.customer-sources.test.tsx src/hooks/useServices.payment-state.test.tsx` | Active-only choices fail closed; selected history stays visible; unchanged inherited attribution is omitted from PATCH; explicit active changes and usage-count normalization stay aligned with the API. |

## Coverage Gaps (Known)

| Domain | Missing Coverage | Risk |
|---|---|---|
| **Payments (backend allocation/void/refund)** | Positive allocation amounts, residual FOR UPDATE, multi-alloc sum, stable reversal locks, and create/PATCH void bans are covered. The required PR Checks PostgreSQL job runs the real two-connection invoice/dotkham matrix through a fail-closed command. Refund balance checks remain thinner. | **Medium** — core allocation/reversal guards are locked; refund math still worth expanding. |
