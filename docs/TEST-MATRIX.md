...
| `api/src/routes/partners/mutationHandlers.js` | `api/src/routes/partners/__tests__/mutationHandlers.test.js` | New dental customers must keep `T######`; new cosmetic customers created through `/api/cosmetic/Partners` must use collision-checked `TM######` refs in the cosmetic DB; partial updates preserve omitted writable UUIDs; normal Partner POST/PUT rejects source assignment/change/clear with `PARTNER_SOURCE_READ_ONLY`; route validation accepts an unchanged UUID regardless of letter case. |
| `api/src/services/sourceChangeAudit.js`, sale-order/partner source mutation paths, migration 075 | `api/tests/sourceChangeAudit.test.js`, `api/tests/sourceChangeAuditMigration.test.js`, `api/src/services/__tests__/sourceChangeAlert.test.js` | INV-026/027: every successful source mutation writes one append-only audit row; reads, rejected locked changes, and other failed writes write none; paid/closed ordinary patches return `409 SOURCE_IMMUTABLE`; unexpected-entry classification/alerting remains defense in depth; reconciliation is bounded; audit UPDATE/DELETE is blocked. |
...
| `api/src/services/exports/builders/legacyFlatReportsExport*.js` | `api/src/services/exports/__tests__/legacyFlatReportsExport.test.js`, `website/e2e/export-downloads.spec.ts` | UC-013, WF-005, INV-023. Locks `revenue-flat` and `deposit-flat` workbook templates, SO-code column mapping, payment/deposit note columns, **separate** order source (`Nguồn đơn`) vs customer source (`Nguồn KH`) with no COALESCE, deposit cash/bank split fallback, posted service-payment filters, allocation proration SQL, row-limit errors, and deposit top-up filtering. |
| `api/src/lib/orderSourceSemantics.js`, `api/src/routes/saleOrders*.js`, `api/src/routes/reports/revenueBreakdowns.js` by-source | `api/src/lib/__tests__/orderSourceSemantics.test.js`, `api/tests/saleOrders.test.js` (source semantics), `api/src/routes/reports/__tests__/revenueRecognition.test.js` (by-source SQL), `website/src/hooks/useServices/mapSaleOrderToServiceRecord.test.ts` | INV-023. Direct order source vs customer source; create snapshot; edit never inherits; closed-period by-source uses `so.sourceid` only. |
| `api/src/services/exports/builders/reportSalesEmployeesExport.js` | `api/src/services/exports/__tests__/reportSalesEmployeesExport.test.js`, `website/src/pages/reports/__tests__/ReportsSubpages.test.tsx` | UC-019, WF-005. Locks `report-sales-employees` preview/download filters, `companyId=all` full extraction, explicit branch rejection when out of scope, employee-type attribution, grouped workbook rows, and `/reports/revenue` export controls. |
| `api/src/services/exports/builders/*Export.js`, `api/src/services/exports/builders/legacyFlat*Query.js`, `api/src/services/exports/investorExportScope.js` | `api/src/services/exports/__tests__/investorExportScope.test.js` | UC-021, INV-021. Locks investor allowlist scoping for customer-derived Excel preview/download builders: customers, services, appointments, payments, revenue-flat, deposit-flat, and report-sales-employees. |
...
| `api/migrations/` customer-source history or migration discovery | `npm --prefix api test -- --runTestsByPath tests/customerSourceMigrationArchiveGuard.test.js --runInBand` (required by `.github/workflows/pr-checks.yml`) | Destructive source rewrites must stay outside both top-level and recursive `*.sql` selection. |
| `scripts/data-repairs/20260723_q10_customer_source_{apply,rollback}.sql` | Restore the verified production backup into a disposable PostgreSQL 16 database; prove refusal without confirmation, apply distribution `21/16/4/2`, zero partner/payment changes, and exact rollback to the captured source IDs | Consequential production repair must be rehearsed against the exact backup before final confirmation. |
| `api/src/db.js`, `api/src/routes/customerSources.js`, `api/src/routes/saleOrders/customerSourceSelection.js`, or sale-order create/update source handling | `npm --prefix api test -- --runTestsByPath tests/dbTransaction.test.js tests/customerSourceIntegrity.test.js tests/customerSourceReferenceMigration.test.js tests/q10CustomerSourceRepairGuard.test.js --runInBand` | New data must reject inactive/missing sources; source management and order writes must serialize on the lookup row; existing history may preserve only its exact source; referenced lookups cannot be deleted or have name/type rewritten (`CUSTOMER_SOURCE_LABEL_LOCKED` / `CUSTOMER_SOURCE_IN_USE`). |
| `api/src/lib/saleOrderSourceLock.js`, `updateSaleOrder.js`, `correctSaleOrderSource.js`, ServiceForm source chips | `npm --prefix api test -- --runTestsByPath tests/saleOrderSourceImmutability.test.js tests/saleOrderSourceNoOpPatch.test.js --runInBand` and `npm --prefix website test -- src/lib/saleOrderSourceLock.test.ts` | Open unpaid orders may change source; paid/closed-period ordinary PATCH is rejected; unrelated edits still work; correction requires `services.source_correct` + full audit fields; a source-only no-op PATCH returns `200` unchanged with zero UPDATE/audit (locked and unlocked), while a truly empty PATCH still returns `400` (INV-026). |
| `api/src/middleware/auth.js` `requireNonInvestorPermission`, or either mounted `/:id/source-correction` route | `npm --prefix api test -- --runTestsByPath tests/sourceCorrectionRouteOrdering.test.js --runInBand` | D21/INV-021 mounted ordering: the guard sits ahead of the handler on both routes; investors get `404` for malformed bodies, valid bodies, with the correction permission, and with `*`, and the handler is never reached; non-investors keep `401`/`403`/pass-through; the mounted path issues no duplicate scope query. Dispatch is in-process (`tests/helpers/dispatchRoute.js`) because express 5 + supertest intermittently returns transport-level `400`s in this environment. |
| `api/migrations/050_add_customer_source_foreign_keys.sql` | Run `customerSourceReferenceMigration.test.js`, then apply twice to a disposable production-backup restore and prove referenced delete/orphan assignment both fail | Referential guards must be idempotent, validated, and restrictive without mutating production during local verification. |
| `website/src/hooks/useSettings.ts`, `website/src/hooks/useServices.ts`, ServiceForm customer-source choices, or `CustomerSourcesConfig` | `npm --prefix website test -- src/hooks/useSettings.customer-sources.test.tsx src/hooks/useServices.payment-state.test.tsx` | Active-only choices fail closed; selected history stays visible; referenced sources stay label-locked in Settings; description/active edits and usage-count normalization stay aligned with the API. |
