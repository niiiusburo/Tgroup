...
| `api/src/lib/saleOrderSourceLock.js`, `updateSaleOrder.js`, `correctSaleOrderSource.js`, ServiceForm source chips | `npm --prefix api test -- --runTestsByPath tests/saleOrderSourceImmutability.test.js --runInBand` and `npm --prefix website test -- src/lib/saleOrderSourceLock.test.ts` | Open unpaid orders may change source; paid/closed-period ordinary PATCH is rejected; unrelated edits still work; correction requires `services.source_correct` + full audit fields. |
