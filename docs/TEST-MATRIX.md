...
| `website/src/pages/Commission.tsx`, `website/src/components/commission/*`, `website/src/pages/Customers.tsx`, `website/src/components/customer/ServiceHistory*.tsx` | `npm --prefix website test -- --run CommissionNavigation NewClientsTab EarningsPayoutsTabs ServiceHistory`, website build/typecheck, live `/commission?tab=newClients&lob=cosmetic` and `/commission?tab=earnings&lob=cosmetic` screenshot smoke | Admin CTV commission navigation must preserve the active tab in the URL, link new-client names to profiles, link earning/payout services to focused customer Records rows, and keep return breadcrumbs intact on mobile. |
...
| `website/src/pages/CTV/*`, `website/src/components/ctv/*`, `website/src/lib/api/ctv.ts`, `website/src/lib/i18n/ctv.ts`, `website/src/i18n/locales/*/ctv.json`, `website/src/components/ui/DatePicker.tsx` when used by the CTV sheet | `npm --prefix website test -- src/components/ctv/CtvReferModal.test.tsx src/components/ui/DatePicker.test.tsx src/__tests__/ProtectedRoute.ctv.test.tsx src/lib/api/__tests__/ctv.booking.test.ts src/pages/CTV/CtvDashboard.test.tsx src/pages/CTV/tabs/CtvTrackingTab.test.tsx`; `npm --prefix website run build`; live `/ctv` desktop and mobile screenshots after deploy; for header/menu edits, include expanded, scroll-hidden, and scroll-returned mobile states; for Me-tab invite edits, verify the visible `/ctv/join?ref=CTV-...` link and copy/share actions; for date-picker edits, include the open refer-client calendar state | CTV users bypass the admin shell and rely on a dedicated mobile-first portal. The Tracking tab depends on `/api/ctv/client-journeys`; booking-sheet errors depend on `B_CLIENT_CLAIMED` compatibility fields; the refer-client date defaults to Vietnam today; the custom in-flow date picker must not trigger native iOS popups or overlap later fields/buttons; language toggle and nullable display fallbacks must remain EN/VI-safe; the compact motion header must free mobile scroll space without hiding primary CTV actions permanently; the Me tab must expose a shareable CTV join URL, not only a raw referral code. |
...
| `website/src/components/calendar/ExportDateRangeModal.tsx`, `website/src/components/appointments/unified/AppointmentFormCore.tsx`, `AppointmentFormShell.tsx`, or shared appointment DatePicker sizing | `npm --prefix website test -- src/components/ui/DatePicker.test.tsx`; `/calendar` screenshot checks with export custom date range open and quick-add appointment date picker open | Calendar export and appointment quick-add date controls must use the in-flow picker and keep Apply/submit footers reachable on mobile. |
