...
| `api/src/routes/ctv.js` self-dashboard reads | `api/src/__tests__/ctvRouteGating.test.js`, `api/src/routes/__tests__/ctvReferrals.test.js`, live `GET /api/ctv/me` as admin must return 403 `S_CTV_ONLY` | CTV commission/referral/profile/hierarchy reads are cross-DB and must remain CTV-only even when admins have broad permissions; referral service rows must stay scoped to the current CTV and hierarchy rows must stay CTV-only. |
| `website/src/pages/CTV/*` and `website/src/components/ctv/*` | `website/src/pages/CTV/index.test.tsx`, `website/src/components/ctv/ReferralFlipCard.test.tsx`, `/ctv` mobile screenshot as a CTV user | CTV portal search/filter/flip-card behavior and CTV hierarchy tab are self-service CTV surfaces and must keep service lists plus upline/downline rows readable on mobile. |
...
| `website/src/hooks/useCustomers.ts`, `website/src/hooks/useAppointments.ts`, `website/src/hooks/useCustomerProfile.ts`, `website/src/hooks/useServices.ts`, `website/src/hooks/usePermissionBoard.ts`, `website/src/lib/api/partners.ts`, `website/src/lib/api/permissions.ts`, `website/src/lib/api/saleOrders.ts`, `api/src/routes/customerBalance.js`, `api/src/server.js` | `website/src/hooks/__tests__/useCustomers.cskh.test.ts`, `website/src/hooks/__tests__/useCustomers.permissions.test.ts`, `website/src/hooks/__tests__/useCustomerProfile.date-normalization.test.tsx`, `website/src/hooks/useServices.payment-state.test.tsx`, `website/src/hooks/usePermissionBoard.lob.test.tsx`, `api/src/__tests__/cosmeticLobGuards.test.js`; live NK3 regression script for the five feedback bugs | NK3 Cosmetic feedback bugs. Locks that customer CRUD, appointments, service records, permissions, and customer balance stay on the active LOB and that `/api/cosmetic/CustomerBalance/:id` remains mounted. |
| `website/src/components/appointments/unified/useAppointmentForm.ts` | `website/src/components/appointments/unified/__tests__/useAppointmentForm.test.ts`; live NK3 UI Calendar/profile appointment-create video check | Shared appointment submit hook. Cosmetic Calendar/profile forms must pass `currentLOB='cosmetic'` into create/update so writes go to `/api/cosmetic/Appointments` instead of dental `/api/Appointments`. |
...
| If you change... | Run these tests... | Why |
|---|---|---|
| `api/src/routes/ctv.js`, `api/src/routes/ctvActions.js`, `api/src/routes/ctvClientJourneys.js`, `api/src/routes/ctvHelpers.js` | `npx jest --runInBand src/routes/__tests__/ctvReferrals.test.js src/routes/__tests__/ctvBookings.test.js src/services/__tests__/referralClaim.test.js src/services/__tests__/referralCard.test.js src/services/__tests__/commissionEngine.test.js`; live/read-only smoke `GET /api/ctv/commission-summary`, `/api/ctv/referrals`, `/api/ctv/client-journeys`, `/api/ctv/me` as a CTV user | CTV routes are the approved cross-DB composition surface. Booking writes must preserve the referral-claim gate, Referral Start card creation, selected LOB DB routing, and per-referral service rows for flip cards. |
| `website/src/pages/CTV/*`, `website/src/components/ctv/*`, `website/src/lib/api/ctv.ts`, `website/src/i18n/locales/*/ctv.json` | `npm --prefix website test -- src/__tests__/ProtectedRoute.ctv.test.tsx src/lib/api/__tests__/ctv.booking.test.ts src/components/ctv/ReferralFlipCard.test.tsx src/pages/CTV/tabs/CtvTrackingTab.test.tsx`; `npm --prefix website run build`; live `/ctv` desktop and mobile screenshots after deploy | CTV users bypass the admin shell and rely on a dedicated mobile-first portal. The Tracking tab depends on `/api/ctv/client-journeys` plus `/api/ctv/referrals.services[]`; booking-sheet errors depend on `B_CLIENT_CLAIMED` compatibility fields. |
...
| If you change... | Run these tests... | Why |
|---|---|---|
| `api/src/routes/auth.js`, `api/src/middleware/auth.js`, `/api/me/lob-scope`, or employee auth-source logic | `NODE_PATH=$PWD/api/node_modules npx jest --runTestsByPath tests/loginRateLimiter.test.js --runInBand` from `api/`, plus live NK3 browser login video for a newly created Cosmetic employee account | Cosmetic-only staff accounts live in `tcosmetic_demo`; auth, `/me`, password changes, and permissions must use the token's source LOB instead of defaulting to Dental. |
