...
| `api/src/services/permissionService.js` | `api/tests/readRoutePermissions.test.js`, `api/src/services/__tests__/permissionService.test.js`, `api/tests/investorIdorScoping.test.js`, `api/tests/investorAdminMutationGuards.test.js`, `api/tests/investorScopeRoutePermissions.test.js`, `website/e2e/permissions-*.spec.ts` | Effective permission divergence causes 403s; investor employees must receive the staff-shell permission set while customer-linked data remains scoped to `dbo.investor_clients`, with empty auth `locations` so the frontend does not over-filter investor pages to a home clinic. |
...
| Investor appointment scoping in `api/src/routes/appointments/*` and report appointment endpoints | `api/src/routes/appointments/__tests__/readHandlers.test.js`, `api/tests/investorIdorScoping.test.js`, `api/tests/investorScopeRoutePermissions.test.js` | Investor employees may view/create/edit only allowlisted customer appointments through the staff-shell permission set; calendar lists must stay all-location while filtering by checked customers. |
...
| Investor customer scoping in `api/src/routes/partners/*`, `api/src/routes/customerBalance.js`, `api/src/routes/customerReceipts.js`, `api/src/routes/dotKhams.js`, `api/src/routes/saleOrders*`, `api/src/routes/saleOrderLines.js`, and `api/src/routes/faceRecognition.js` | `api/tests/investorIdorScoping.test.js`, `api/tests/investorScopeRoutePermissions.test.js`, `api/src/services/__tests__/permissionService.test.js`, `api/tests/faceRecognition.test.js` | Investor employees must see/mutate/recognize only allowlisted customers and get 403/404/empty results for all other customer-linked records. |
| Admin investor visibility curation in `api/src/routes/partners/investorVisibility.js`, `website/src/pages/Customers/CustomerColumns.tsx`, and `website/src/pages/Customers/useInvestorVisibility.ts` | `api/src/routes/partners/__tests__/investorVisibility.test.js`, `api/tests/readRoutePermissions.test.js`, `website/src/pages/Customers/CustomerColumns.test.tsx` | Only Admin-class staff can see or change the customer Investor checkbox; investor users remain scoped and cannot self-curate. |
...
| `api/src/routes/faceRecognition.js`, `api/src/services/faceMatchEngine.js`, `api/src/services/comprefaceFaceProvider.js`, `api/src/services/faceReadinessScore.js`, `website/src/components/shared/GlobalFaceIdButton.tsx`, `website/src/components/customer/CustomerProfile/ProfileHeader.tsx`, `website/src/hooks/useFaceRecognition.ts`, `website/src/hooks/useCustomerProfile.ts`, `website/src/lib/api/partners.ts` | `api/tests/faceRecognition.test.js`, `api/src/services/__tests__/faceMatchEngine.test.js`, `api/src/services/__tests__/comprefaceFaceProvider.test.js`, `api/src/services/__tests__/faceReadinessScore.test.js`, `website/src/components/shared/GlobalFaceIdButton.test.tsx`, `website/src/components/customer/CustomerProfile.test.tsx`, `website/src/hooks/__tests__/useFaceRecognition.test.ts`, `website/src/hooks/__tests__/useCustomerProfile.date-normalization.test.tsx`, `website/src/lib/api/__tests__/faceRecognition.test.ts` | Face registration, re-registration, recognition, provider routing, strict threshold/margin policy, staff-visible Face ID version metadata, profile readiness scoring/display, and rescan-only ambiguous header results. |
...
| Investor payment read/write filters in `api/src/routes/payments*.js`, `api/src/routes/accountPayments.js`, `api/src/routes/receipts.js`, `api/src/routes/monthlyPlans.js` | `api/tests/paymentsLegacyFallback.test.js`, `api/tests/investorIdorScoping.test.js`, `api/tests/investorScopeRoutePermissions.test.js`, `api/src/routes/reports/__tests__/cashFlow.test.js` | Investors may use payment staff-shell controls only against allowlisted customer payment context, including legacy `accountpayments` fallback reads. |
...
| If you change... | Run these tests... | Why |
|---|---|---|
| `api/src/routes/exports.js` | `website/e2e/export-downloads.spec.ts`, `api/src/services/exports/__tests__/exportScope.test.js`, `api/src/services/exports/__tests__/legacyFlatReportsExport.test.js`, `api/src/services/exports/__tests__/reportSalesEmployeesExport.test.js` | UC-013/UC-019, WF-005. Locks `CON-Exports-Preview` and `CON-Exports-Download` as current `POST /api/Exports/:type/...` contracts plus backend scope and workbook generation. |
...
| `api/src/services/exports/builders/reportExportScope.js` | `api/src/services/exports/__tests__/exportScope.test.js`, `api/src/services/exports/__tests__/appointmentsExport.test.js`, `api/src/services/exports/__tests__/legacyFlatReportsExport.test.js`, `api/src/services/exports/__tests__/reportSalesEmployeesExport.test.js` | INV-021/INV-023. Shared Excel extraction scope must narrow omitted/all branch filters to allowed locations, reject explicit out-of-scope branches, and apply investor checked-customer filters before row queries. |
...
| Employee location scoping in `api/src/services/reportLocationScope.js`, `api/src/routes/reports/*.js`, `api/src/services/reports/canonicalRevenue.js`, and operational export builders | `api/src/routes/reports/__tests__/locationScope.test.js`, `api/src/services/exports/__tests__/exportScope.test.js`, `api/src/services/exports/__tests__/legacyFlatReportsExport.test.js`, `api/src/services/exports/__tests__/reportSalesEmployeesExport.test.js`, `website/src/pages/reports/__tests__/ReportsLocationScope.test.tsx` | INV-023. Scoped employees must see only allowed company IDs when `companyId` is empty/all, unauthorized explicit branch requests must fail with 403, Excel previews/downloads must not leak other branches, and the Reports UI must lock single-location employees to their assigned branch. |
...
| `api/src/routes/reports/cashFlow.js` | `api/src/routes/reports/__tests__/cashFlow.test.js` | UC-013, WF-013. Locks `CON-Reports-CashFlowSummary`, service collections vs deposits/refunds/deposit usage/voided rows, route mounting, timezone-safe date buckets, investor branch filters composed with checked-customer scope, and scoped location rejection. |
...
| Investor report and Excel scoping in `api/src/routes/reports/*.js`, `api/src/routes/dashboardReports.js`, `api/src/services/reports/canonicalRevenue.js`, and customer-linked export builders | `api/tests/investorIdorScoping.test.js`, `api/src/routes/reports/__tests__/locationScope.test.js`, `api/src/routes/reports/__tests__/cashFlow.test.js`, `api/src/routes/reports/__tests__/revenueRecognition.test.js`, `api/src/routes/reports/__tests__/servicesBreakdown.test.js`, `api/src/services/exports/__tests__/exportScope.test.js`, `api/src/services/exports/__tests__/legacyFlatReportsExport.test.js`, `api/src/services/exports/__tests__/reportSalesEmployeesExport.test.js` | Investor aggregates and Excel rows must count only allowlisted customers while preserving branch/date filters and canonical revenue formulas for normal staff. |
...
| `website/src/components/shared/FaceCaptureModal.tsx` / `website/src/components/shared/useFaceCaptureController.ts` / `website/src/components/shared/faceCaptureEngine.ts` | `website/src/components/shared/FaceCaptureModal.test.tsx`, `website/src/components/shared/faceCaptureEngine.test.ts`, `website/src/components/customer/CustomerCameraWidget.test.tsx`, `website/src/components/shared/GlobalFaceIdButton.test.tsx`, `website/src/hooks/__tests__/useFaceRecognition.test.ts` | Camera lifetime, iOS-friendly camera constraints, light overlay-only privacy blur, no-face messaging, auto-capture gating, video playback blocks, and caller error propagation. |
| `api/src/routes/faceRecognition.js`, `api/src/services/faceMatchEngine.js`, `api/src/services/comprefaceFaceProvider.js`, `website/src/components/shared/GlobalFaceIdButton.tsx`, `website/src/hooks/useFaceRecognition.ts`, `website/src/lib/api/partners.ts` | `api/tests/faceRecognition.test.js`, `api/src/services/__tests__/faceMatchEngine.test.js`, `api/src/services/__tests__/comprefaceFaceProvider.test.js`, `website/src/components/shared/GlobalFaceIdButton.test.tsx`, `website/src/hooks/__tests__/useFaceRecognition.test.ts`, `website/src/lib/api/__tests__/faceRecognition.test.ts` | Face register/re-register/recognize API contract in local and CompreFace modes, strict threshold/margin policy, staff-visible recognition version metadata, rescan-only ambiguous header results, guided straight/left/right no-match rescue registration, plus hidden diagnostics recording without leaking private diagnostics in the staff response. |
| `api/src/routes/faceCheckin.js` / `website/src/pages/CheckIn/CheckIn.tsx` | `api/src/routes/__tests__/faceCheckin.test.js`, `website/src/pages/CheckIn/CheckIn.test.tsx`, `website/src/components/shared/faceCaptureEngine.test.ts` | Public no-auth kiosk route, front-camera startup, visible camera flip, light overlay-only privacy preview, unblurred provider capture, transient no-face retry scanning, privacy-minimized match response, rate limiting, hidden diagnostic recording, and public page rendering without AuthProvider. |
...
# Validate: staff header and camera popup show the current NK2 Face ID version badge, recognition responses carry recognitionVersion, and ambiguous header results are rescan-only
