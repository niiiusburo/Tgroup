...
| If you change... | Run these tests... | Why |
|---|---|---|
| `api/src/routes/ctv.js`, `api/src/routes/ctvActions.js`, `api/src/routes/ctvClientJourneys.js`, `api/src/routes/ctvHelpers.js` | `npx jest --runInBand src/routes/__tests__/ctvBookings.test.js src/services/__tests__/referralClaim.test.js src/services/__tests__/referralCard.test.js src/services/__tests__/commissionEngine.test.js`; live/read-only smoke `GET /api/ctv/commission-summary`, `/api/ctv/referrals`, `/api/ctv/client-journeys`, `/api/ctv/me` as a CTV user | CTV routes are the approved cross-DB composition surface. Booking writes must preserve the referral-claim gate, Referral Start card creation, and selected LOB DB routing. |
| `website/src/pages/CTV/*`, `website/src/components/ctv/*`, `website/src/lib/api/ctv.ts`, `website/src/i18n/locales/*/ctv.json` | `npm --prefix website test -- src/__tests__/ProtectedRoute.ctv.test.tsx src/lib/api/__tests__/ctv.booking.test.ts`; `npm --prefix website run build`; live `/ctv` desktop and mobile screenshots after deploy | CTV users bypass the admin shell and rely on a dedicated mobile-first portal. The Tracking tab depends on `/api/ctv/client-journeys`; booking-sheet errors depend on `B_CLIENT_CLAIMED` compatibility fields. |
