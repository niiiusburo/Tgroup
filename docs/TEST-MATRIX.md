...
| `nginx.conf` or `nginx.docker.conf` | `api/tests/nginxTimeouts.test.js`, `website/e2e/export-downloads.spec.ts` (large dataset) | INV-019 `/api` proxy 300s timeouts (AUD-013) and long-running export behavior. |
| `api/src/server.js` `GET /api/health`, `api/src/services/faceRecognitionRuntime.js`, `api/src/services/faceEngineClient.js`, `api/src/services/comprefaceClient.js` | `api/tests/health.test.js`, `api/src/services/__tests__/faceRecognitionRuntime.test.js`, `api/src/services/__tests__/faceEngineClient.test.js`, `api/src/services/__tests__/comprefaceClient.test.js` | DB-required vs optional face degrade, including bounded and aborted never-settling local and CompreFace probes with signal propagation to provider fetches (INV-014 / AUD-038). |
