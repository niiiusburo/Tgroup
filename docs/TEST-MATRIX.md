...
| `website/src/pages/CTV/*`, `website/src/components/ctv/*`, `website/src/lib/api/ctv.ts`, `website/src/lib/i18n/ctv.ts`, `website/src/i18n/locales/*/ctv.json` | `npm --prefix website test -- src/components/ctv/CtvReferModal.test.tsx src/__tests__/ProtectedRoute.ctv.test.tsx src/lib/api/__tests__/ctv.booking.test.ts src/pages/CTV/CtvDashboard.test.tsx src/pages/CTV/tabs/CtvTrackingTab.test.tsx`; `npm --prefix website run build`; live `/ctv` desktop and mobile screenshots after deploy; for header/menu edits, include expanded, scroll-hidden, and scroll-returned mobile states; for Me-tab invite edits, verify the visible `/ctv/join?ref=CTV-...` link and copy/share actions | CTV users bypass the admin shell and rely on a dedicated mobile-first portal. The Tracking tab depends on `/api/ctv/client-journeys`; booking-sheet errors depend on `B_CLIENT_CLAIMED` compatibility fields; the refer-client date defaults to Vietnam today; language toggle and nullable display fallbacks must remain EN/VI-safe; the compact motion header must free mobile scroll space without hiding primary CTV actions permanently; the Me tab must expose a shareable CTV join URL, not only a raw referral code. |
