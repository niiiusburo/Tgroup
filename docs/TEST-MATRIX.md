...
| `api/src/services/permissionService.js` | `api/src/services/__tests__/permissionService.test.js`, `api/tests/readRoutePermissions.test.js`, `api/tests/investorIdorScoping.test.js`, `api/tests/investorScopeRoutePermissions.test.js`, `api/tests/externalCheckupsInvestorScope.test.js`, `api/tests/stockPickingsInvestorScope.test.js`, `api/tests/investorVisibilityCompatibility.test.js`, `website/e2e/permissions-*.spec.ts` | Effective permission divergence causes 403s; investor scope must fail closed to allowlisted customers, preserve legacy account-keyed allowlist rows, and match live `Investor` group casing. |
| `api/src/routes/externalCheckups.js`, `api/src/services/hosoonlineClient.js` | `api/tests/externalCheckupsInvestorScope.test.js`, `api/src/routes/__tests__/externalCheckups.test.js` | AUD-005 / INV-021. Investor IDOR on customer-code list, image-by-name proxy (including mismatched query and filename customer codes), patient create, and health-checkup upload; staff unaffected. |
| `api/src/routes/stockPickings.js` | `api/tests/stockPickingsInvestorScope.test.js`, `api/tests/investorScopeRoutePermissions.test.js` | AUD-005 / INV-021. StockPickings GET requires `settings.view` and partner allowlist filter; detail 404 outside allowlist. |
