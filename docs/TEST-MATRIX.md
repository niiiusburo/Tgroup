...
| `website/src/pages/CTV/*` and `website/src/components/ctv/*` | `website/src/pages/CTV/CtvDashboard.test.tsx`, `website/src/components/ctv/ReferralFlipCard.test.tsx`, `/ctv` mobile screenshot as a CTV user | CTV portal bottom menu, search/filter/flip-card behavior, and Network hierarchy tab are self-service CTV surfaces and must keep service lists plus upline/downline rows readable on mobile. |
