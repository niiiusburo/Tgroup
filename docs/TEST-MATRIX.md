...
| If you change... | Run these tests... | Why |
|---|---|---|
| `api/scripts/import-nk3-cosmetic-catalog.js` | `npx jest --runTestsByPath api/tests/nk3CosmeticCatalogImport.test.js --runInBand`; `node api/scripts/import-nk3-cosmetic-catalog.js --parse-only --extract`; `node api/scripts/import-nk3-cosmetic-catalog.js --dry-run --target local`; scoped Semgrep on the script/test | NK3 cosmetic catalog import writes products, categories, companies, and company partner rows in the cosmetic DB only: local `tcosmetic_demo`, or NK3 online `tcosmetic_smoketest` when `--target nk3`. It must keep Vietnamese tone-distinct services separate, reject NK/NK2/dental targets, skip row 66, and require explicit confirmation before NK3 apply. |
| `api/src/routes/products.js` or `api/src/routes/productCategories.js` under `/api/cosmetic/*` | Above NK3 importer tests + `api/src/__tests__/cosmeticLobGuards.test.js` + service catalog UI smoke after NK3 apply | Cosmetic service catalog data must stay inside the cosmetic DB and remain gated by `requireLobScope('cosmetic')`. |
