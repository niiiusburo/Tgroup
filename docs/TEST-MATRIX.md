...
| `website/src/pages/CTV/*`, `website/src/components/ctv/*`, `website/src/lib/api/ctv.ts`, `website/src/lib/i18n/ctv.ts`, `website/src/i18n/locales/*/ctv.json` | `npm --prefix website test -- src/__tests__/ProtectedRoute.ctv.test.tsx src/lib/api/__tests__/ctv.booking.test.ts src/pages/CTV/CtvDashboard.test.tsx src/pages/CTV/tabs/CtvTrackingTab.test.tsx`; `npm --prefix website run build`; live `/ctv` desktop and mobile screenshots after deploy | CTV users bypass the admin shell and rely on a dedicated mobile-first portal. The Tracking tab depends on `/api/ctv/client-journeys`; booking-sheet errors depend on `B_CLIENT_CLAIMED` compatibility fields; language toggle and nullable display fallbacks must remain EN/VI-safe. |
