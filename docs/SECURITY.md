...
Investor customer scope is an additional server-side filter. If an authenticated user resolves to the `investor` group, customer-derived routes must apply `dbo.investor_clients` and return empty lists or 404s for non-allowlisted customers without disclosing existence. This includes ExternalCheckups (customer code + image-by-name proxy, cross-validating a resolvable query code and filename code) and StockPickings GET (partner-scoped list/detail behind `settings.view`).
