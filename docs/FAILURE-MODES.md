...
- **Root Cause:** ExternalCheckups image proxy and StockPickings GET only enforced auth/permission (or auth alone) and never called `resolveInvestorScope`. Image URLs had no customer binding, so allowlisted list responses were not required to open bytes.
- **Fix:** Apply INV-021 fail-closed checks on ExternalCheckups customer-code reads/mutations and image-by-name (query `customerCode` + filename T-code extraction); gate StockPickings GET with `settings.view` and partner allowlist filters.
- **Prevention:** `api/tests/externalCheckupsInvestorScope.test.js`, `api/tests/stockPickingsInvestorScope.test.js`, and `investorScopeRoutePermissions` StockPickings gates.
