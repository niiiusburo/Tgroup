...
## FM-20260724-01: Settings Rename Rewrites Historical Source Labels

- **Symptom:** Editing a customer-source name or type in Settings changes `Nguồn khách` on already-exported closed-period revenue/service workbooks even though no order was edited.
- **Root Cause:** `PUT /api/CustomerSources/:id` updated `customersources.name`/`type` in place. Reports and exports join attribution IDs to the live lookup row, so a taxonomy rename rewrote every historical label that still pointed at that ID.
- **Fix:** While any `partners.sourceid` or `saleorders.sourceid` reference remains, block name/type mutation with `400 CUSTOMER_SOURCE_LABEL_LOCKED` and block delete with `400 CUSTOMER_SOURCE_IN_USE`. Allow only `description` and `is_active`. Semantic changes create a new source row.
- **Prevention:** `customerSourceIntegrity.test.js` covers referenced name/type rejection, safe deactivation/description edits, unreferenced renames, and delete-in-use. Settings UI surfaces the lock and steers staff to create a new source.
- **Related:** INV-023, INV-024, FM-20260723-01, UC-009, UC-013, WF-005.
